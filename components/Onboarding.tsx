    ...
    <div className={`h-[100dvh] flex flex-col p-8 pt-safe pb-safe transition-colors duration-500 overflow-hidden ${currentStep.color}`}>
      <div className="flex-1 flex flex-col justify-center items-center text-center">
        ...
        <div className={`mt-6 transition-all duration-300 ${step === 3 ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
          <div className="bg-white/60 px-5 py-2 rounded-full border border-blue-100 flex items-center gap-2 backdrop-blur-sm shadow-sm">
      ...
      <div className="shrink-0 space-y-4 pt-4">
        ...
        <button 
          ...
          className="w-full bg-indigo-600 text-white font-black py-5 rounded-[24px] shadow-xl shadow-indigo-100 active:scale-95 active:bg-indigo-700 transition-all text-sm uppercase tracking-widest min-h-[64px]"
        >
        ...
        <button 
          onClick={onComplete}
          className={`w-full text-slate-400 font-black text-xs uppercase tracking-widest py-2 active:text-slate-600 transition-all ${step === steps.length - 1 ? 'invisible pointer-events-none' : 'visible'}`}
        >
          Skip Intro
        </button>
      </div>
